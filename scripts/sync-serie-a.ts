...
async function main() {
  console.log("⚽ מסנכרן סרייה א' מ-TheSportsDB...")

  const tournament = await db.tournament.upsert({
    where: { slug: SLUG },
    update: { isActive: true },
    create: { name: 'Serie A', nameHe: "סרייה א'", slug: SLUG, type: 'serie_a', isActive: true, season: SEASON },
  })
  ...
  for (const e of events) {
    ...
    const match = await db.match.upsert({
      ...
      create: { tournamentId: tournament.id, homeTeamId, awayTeamId, kickoffAt, lockAt, status, homeScore, awayScore, providerMatchId, round: e.intRound ? `Giornata ${e.intRound}` : e.strRound || undefined },
    })
  ...
  console.log("🎉 סנכרון סרייה א' הושלם!")
}
