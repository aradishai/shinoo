...
const LEAGUE_ID = 4717
const SEASON = '2025-2026'
const SLUG = 'ligat-haal-2025-2026'
...
async function main() {
  console.log('⚽ מסנכרן ליגת העל מ-TheSportsDB...')

  const tournament = await db.tournament.upsert({
    where: { slug: SLUG },
    update: { isActive: true },
    create: { name: 'Ligat HaAl', nameHe: 'ליגת העל', slug: SLUG, type: 'ligat_haal', isActive: true, season: SEASON },
  })
  ...
  for (const e of events) {
    ...
    const match = await db.match.upsert({
      ...
      create: { tournamentId: tournament.id, homeTeamId, awayTeamId, kickoffAt, lockAt, status, homeScore, awayScore, providerMatchId, round: e.intRound ? `מחזור ${e.intRound}` : e.strRound || undefined },
    })
  ...
  console.log('🎉 סנכרון ליגת העל הושלם!')
}
