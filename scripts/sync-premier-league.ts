...
const SLUG = 'premier-league-2025-2026'
...
async function main() {
  console.log('⚽ מסנכרן פרמייר ליג מ-TheSportsDB...')

  const tournament = await db.tournament.upsert({
    where: { slug: SLUG },
    update: { isActive: true },
    create: { name: 'Premier League', nameHe: 'פרמייר ליג', slug: SLUG, type: 'premier_league', isActive: true, season: SEASON },
  })
  ...
  console.log('🎉 סנכרון פרמייר ליג הושלם!')
}
