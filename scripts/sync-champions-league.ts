...
const SLUG = 'champions-league-2025-2026'
...
async function main() {
  console.log('⚽ מסנכרן ליגת האלופות מ-TheSportsDB...')

  const tournament = await db.tournament.upsert({
    where: { slug: SLUG },
    update: { isActive: true },
    create: { name: 'UEFA Champions League', nameHe: 'ליגת האלופות', slug: SLUG, type: 'champions_league', isActive: true, season: SEASON },
  })
  ...
  for (const e of events) {
    ...
    const match = await db.match.upsert({
      ...
      create: { tournamentId: tournament.id, homeTeamId, awayTeamId, kickoffAt, lockAt, status, homeScore, awayScore, providerMatchId, round: e.intRound ? `Matchday ${e.intRound}` : e.strRound || undefined },
    })
  ...
  console.log('🎉 סנכרון ליגת האלופות הושלם!')
}
