...
export function Onboarding({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-dark flex flex-col items-center justify-between px-6 py-12" dir="rtl">
      {/* Skip */}
      ...
      {/* Slide content */}
      <div className="flex flex-col items-center text-center gap-5 flex-1 justify-center w-full max-w-xs">
        {slide.logo && (
          <img src="/shinoo-title.png" alt="SHINOO" className="h-28 w-auto" style={{ mixBlendMode: 'lighten' }} />
        )}
        {slide.emoji && (
            ...
            {slide.powers?.map((p) => (
              <div key={p.name} className="flex flex-col items-center gap-2">
                <div className="w-16 h-16 rounded-2xl bg-dark-card border border-dark-border flex items-center justify-center">
                  <img src={p.img} alt={p.name} className="w-12 h-12 object-contain" style={{ mixBlendMode: 'lighten' }} />
                </div>
        ...
        <h2 className="text-white font-black text-2xl">{slide.title}</h2>
        <p className="text-gray-400 text-sm leading-relaxed">{slide.body}</p>

        {slide.points && (
          <div className="w-full space-y-2 mt-1">
            {slide.points.map((p) => (
              <div key={p.label} className="flex items-center justify-between bg-dark-card border border-dark-border rounded-xl px-4 py-2.5">
        ...
        {slide.powers && (
          <div className="w-full space-y-2 mt-1">
            {slide.powers.map((p) => (
              <div key={p.name} className="flex items-center justify-between bg-dark-card border border-dark-border rounded-xl px-4 py-2.5">
