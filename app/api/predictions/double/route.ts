...
export async function DELETE(request: Request) {
  ...
  // Once chatSent, the entry is locked — no changes allowed
  if (entry.chatSent)
    return NextResponse.json({ error: 'DOUBLE נעול — המשחק הראשון כבר התחיל' }, { status: 400 })

  if (slot === 1) {
    if (entry.predictionId1) {
      const pred = await db.prediction.findUnique({ where: { id: entry.predictionId1 }, include: { match: true } })
    ...
    if (entry.predictionId2) {
      const pred = await db.prediction.findUnique({ where: { id: entry.predictionId2 }, include: { match: true } })
      if (pred && (pred.match.status !== 'SCHEDULED' || new Date() >= pred.match.lockAt))
        return NextResponse.json({ error: 'המשחק נעול — לא ניתן לשנות' }, { status: 400 })
    }
    // Also block if slot 1 match is locked
    if (entry.predictionId1) {
      const pred1 = await db.prediction.findUnique({ where: { id: entry.predictionId1 }, include: { match: true } })
      if (pred1 && (pred1.match.status !== 'SCHEDULED' || new Date() >= pred1.match.lockAt))
        return NextResponse.json({ error: 'המשחק הראשון נעול — לא ניתן לשנות' }, { status: 400 })
    }
