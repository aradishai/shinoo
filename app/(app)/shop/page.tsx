...
const SHOP_ITEMS = [
  ...
  { id: 'x3', name: 'כפול 3', description: 'לשימוש לפני המשחק – שילוש ניקוד המשחק', img: '/btn-x3.png', stockKey: 'x3Stock' as const, comingSoon: false, cost: 8 },
  ...
  { id: 'peek', name: 'PEEK', description: 'לשימוש לפני נעילת המשחק – ראה את ניחושי כל השחקנים ונשאר פתוח לניחוש 30 דקות נוספות', img: '/btn-peek.png', stockKey: 'peekStock' as const, comingSoon: false, cost: 2, imgClass: 'w-44 h-12' },
  { id: 'et120', name: '120 ET', description: 'לשימוש במהלך המשחק עד דקה 75. הופך את הניחוש שלך ל-120 במקום ל-90\'', img: '/btn-et120.png', stockKey: 'et120Stock' as const, comingSoon: false, cost: 2, imgClass: 'w-36 h-12' },
]
    ...
    <div className="px-4 py-6">
      ...
      <div ref={tooltipRef} className="space-y-5">
        {SHOP_ITEMS.filter(item => !(item as any).hidden && (item.id !== 'x3' || lowerPlayoff)).map((item, i) => {
          ...
          return (
            <div key={item.id}>
              <div className={`flex items-center overflow-hidden ${item.comingSoon ? 'opacity-40' : ''}`}>

                {/* Logo + ? */}
                <div className="flex-1 flex items-center justify-start gap-2 pl-2 min-w-0">
                  ...
                  {!item.comingSoon && (
                    <button
                      ...
                      className={`w-6 h-6 rounded-full border text-xs font-bold flex items-center justify-center transition-colors ${showTooltip ? 'border-white text-white' : 'border-gray-600 text-gray-400'}`}
                    >
                      ?
                    </button>
                  )}
              ...
              {showTooltip && (
                <p className="text-gray-400 text-xs mt-2 px-1 text-right">{item.description}</p>
              )}
              {i < SHOP_ITEMS.length - 1 && <div className="w-full h-px bg-dark-border mt-4" />}
            </div>
          )
        })}
      </div>
